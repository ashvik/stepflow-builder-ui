...
const StepFlowBuilderAppV3: React.FC = () => {
...
viewMode: 'tabs' // Always use multi-tab mode
  ...
  const [activeTab, setActiveTab] = useState<'workflow' | 'configuration' | 'yaml' | 'properties'>('yaml')
  ...
  return (
    <div className={`w-full h-full flex flex-col bg-background ${darkMode ? 'dark' : ''}`}>
      {/* Header */}
      <div className="flex items-center justify-between p-3 border-b border-border bg-card">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <LayoutDashboard className="w-6 h-6 text-primary" />
            <h1 className="text-xl font-semibold">StepFlow Builder V3</h1>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="icon" variant="outline" onClick={() => setDarkMode(!darkMode)} title="Toggle Theme">
                  ...
                  <div className="relative w-full h-full">
                    ...
                    >
                      ...
                      >
                        {(() => {
                          try {
                            const yaml = YAML.stringify(appState.config, { indent: 2, lineWidth: -1, minContentWidth: 0 })
                            return yaml.split('\n').map((line, index) => (
...
export default StepFlowBuilderAppV3
